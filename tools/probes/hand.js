...
// The stylesheet takes a snapped link to `opacity: 1` and the nav's own 0.3s ease
// carries it there. Virtual time will not run a CSS transition to its end however
// long this settles, so the reading drifted between 0.78 and 0.87 across runs and
// the assertion was flaky rather than wrong. Killing the transition first makes the
// computed value the declared one, which is what is actually being asserted.
...
const pass = {
  ...
  snapIsNotDimmed: snapped.opacity === 1,
